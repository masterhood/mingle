import rally from './helpers/setup';

test.serial('rally.react()', t => {
    let response = rally.react('resources/assets/js/app.js', 'public/js');

    t.is(rally, response);

    t.deepEqual(
        [
            {
                entry: [new File('resources/assets/js/app.js')],
                output: new File('public/js')
            }
        ],
        Rally.components.get('react').toCompile
    );
});

test.cb.serial('it compiles React and a preprocessor properly', t => {
    rally
        .react('test/fixtures/fake-app/resources/assets/js/app.js', 'js')
        .sass('test/fixtures/fake-app/resources/assets/sass/app.scss', 'css');

    compile(t, config => {
        assertManifestIs(
            {
                '/js/app.js': '/js/app.js',
                '/css/app.css': '/css/app.css'
            },
            t
        );

        t.true(File.exists('test/fixtures/fake-app/public/js/app.js'));
    });
});

test.serial('it sets the webpack entry correctly', t => {
    rally.react('resources/assets/js/app.js', 'js');

    t.deepEqual(
        {
            '/js/app': [path.resolve('resources/assets/js/app.js')]
        },
        buildConfig().entry
    );
});

test.serial('it sets the babel config correctly', t => {
    rally.react('resources/assets/js/app.js', 'js');

    buildConfig();

    t.true(Config.babel().presets.includes('react'));
});
