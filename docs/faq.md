...
### Does this tool require that I use Ofcold?

No. It has a few optimizations for Ofcold, but it can be used for any project.
...
It's highly recommended that you add the following NPM scripts to your `package.json` file. Please note that Ofcold includes these out of the box.
...
Now you may [configure](https://github.com/adrienkohlbecker/vagrant-fsnotify#basic-setup) vagrant to use the plugin. In [Homestead](https://ofcold.com/docs/5.4/homestead), your `Homestead.yaml` file would look something like this:

```yaml
folders:
    -
        map: /Users/jeffrey/Code/ofcold
        to: /home/vagrant/Code/ofcold
        options:
            fsnotify: true
            exclude:
                - node_modules
...
2. Add the following to your `webpack.rally.js` file to disable CSS url() processing.
...
### My rally-manifest.json file shouldn't be in the project root.

If you're not using Ofcold, your `rally-manifest.json` file will be dumped into the project root. If you need to change this, call `rally.setPublicPath('dist/');`, and your manifest file will now be saved in that base directory.
...
While Ofcold Rally automatically loads jQuery for you (exactly as the example above demonstrates), should you need to disable it (by passing an empty object), or override it with your own modules, you may use the `rally.autoload()` method. Here's an example:
