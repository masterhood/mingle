...
module.exports = function () {
    ...
    // Here, we'll filter through all CSS preprocessors that the user has requested. For each one, we'll add a new Webpack
    // rule and then prepare the necessary extract plugin to extract the CSS into its file.
    ...
    // If there were no existing extract text plugins to add our Vue styles extraction too, we'll push a new one in.
    if (Config.extractVueStyles && ! extractPlugins.length) {
        extractPlugins.push(vueExtractPlugin);
    }

    // If we want to import a global styles file in every component, use sass resources loader
