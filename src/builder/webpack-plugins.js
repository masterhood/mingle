...
let RallyDefinitionsPlugin = require('../webpackPlugins/RallyDefinitionsPlugin');
...
module.exports = function() {
    ...
    // Activate support for Rally_ .env definitions.
    plugins.push(
        RallyDefinitionsPlugin.build({
            NODE_ENV: Rally.inProduction()
                ? 'production'
                : process.env.NODE_ENV || 'development'
        })
    );

    if (!Rally.components.get('version') && Rally.isUsing('hmr')) {
    ...
    plugins.push(
        new BuildCallbackPlugin(stats => Rally.dispatch('build', stats))
    );
